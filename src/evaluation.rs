...
// PeSTO material values (Rofchade-tuned)
...
// Game phase contribution per piece type (max = 24)
// Knight=1, Bishop=1, Rook=2, Queen=4, Pawn/King=0
...
fn score_for(board: &Board, color: Color) -> (i32, i32, i32) {
    let mut mg = 0;
    let mut eg = 0;
    let mut phase = 0;
    ...
    for (i, &pt) in piece_types.iter().enumerate() {
        for sq in board.colored_pieces(color, pt) {
            ...
            mg += MG_VALUE[i] + MG_PST[i][idx];
            eg += EG_VALUE[i] + EG_PST[i][idx];
            phase += PHASE_INC[i];
        }
    }
...
pub fn evaluate(board: &Board) -> i32 {
    let (white_mg, white_eg, white_phase) = score_for(board, Color::White);
    let (black_mg, black_eg, black_phase) = score_for(board, Color::Black);

    let mg_score = white_mg - black_mg;
    let eg_score = white_eg - black_eg;

    let mg_phase = (white_phase + black_phase).min(24);
